        ...
        <>
            ...
            <div className={`fixed top-0 right-0 h-full w-[320px] sm:w-[380px] bg-[#F4F4F0] border-l-[3px] border-black z-50 transform transition-transform duration-300 ease-out shadow-[-10px_0px_0px_0px_rgba(0,0,0,1)] ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="p-4 flex items-center justify-between border-b-[3px] border-black bg-white">
                    <div>
                        <h2 className="font-[Unbounded] text-2xl font-black uppercase">YOUR STACK ({cartItemCount})</h2>
                        {cartItems.length > 0 && (
                            <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-gray-500">Expires in {timeRemaining}</p>
                        )}
                    </div>
                            ...
                            return (
                                ...
                                <div className="flex flex-col justify-between flex-grow">
                                    <div>
                                        ...
                                        <div className="mt-2 flex items-center justify-between text-[11px] font-bold uppercase tracking-[0.3em] text-dark/50">
                                            <span>Each {formatCurrency(item.price)}</span>
                                            <span className="font-[Unbounded] text-base text-dark">{formatCurrency(lineTotal)}</span>
                                        </div>
                                    </div>
                ...
                <div className="absolute bottom-0 left-0 w-full p-4 bg-[#F4F4F0] border-t-[3px] border-black">
                    <div className="flex justify-between items-end mb-4">
                        ...
                        <span className="font-[Unbounded] text-2xl font-black">{cartItemCount}</span>
                    </div>
                    <div className="flex justify-between items-end mb-4">
                        <span className="font-[Space_Grotesk] font-bold uppercase text-sm">Subtotal</span>
                        <span className="font-[Unbounded] text-2xl font-black">{formatCurrency(totalPrice)}</span>
                    </div>
                    <button
